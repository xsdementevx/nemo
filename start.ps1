...
Function Sync-LocalTime {
    ...
    Write-Host "time sync with $NtpServer..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "time sync success" -ForegroundColor Green
        return $true
    }
    catch {
        Write-Host "try alternative method of time sync..." -ForegroundColor Yellow
        try {
            ...
            Write-Error "time sync error: $($_.Exception.Message)"
            return $false
        }
    }
}
